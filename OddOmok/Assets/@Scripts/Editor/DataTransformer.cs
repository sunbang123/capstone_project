...
public class DataTransformer : EditorWindow
{
    ...
    static void LEGACY_ParseTestData(string filename)
    {
        ...
        for (int y = 1; y < lines.Length; y++)
        {
            ...
            testData.BlackGamePlayed = ConvertValue<int>(row[i++]);
            testData.WhiteGamePlayed = ConvertValue<int>(row[i++]);
            testData.BlackWins = ConvertValue<int>(row[i++]);
            testData.WhiteWins = ConvertValue<int>(row[i++]);
